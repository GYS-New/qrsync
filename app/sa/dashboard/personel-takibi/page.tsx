...
export default async function SAPersonelTakibiPage() {
  ...
  if (!firmaId) {
    return (
      <div>
        <Topbar title="Personel Takibi" base="/sa" breadcrumbs={[{ label: 'Yönetim' }, { label: 'Personel Takibi' }]} />
        <div className="verde-card" style={{ margin: 24, padding: 24, color: '#7a907a', fontSize: 14 }}>
          Personel takibini görmek için üstten bir firma seçin.
        </div>
      </div>
    )
  }
  ...
  const userIds = (kullanicilar ?? []).map((u: any) => u.id)
  ...
  return (
    <div>
      <Topbar
        ...
        base="/sa"
        breadcrumbs={[{ label: 'Yönetim' }, { label: aktifProje?.ad ?? 'Tüm Firma' }, { label: 'Personel Takibi' }]}
      />
      <div style={{ padding: 24 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 20 }}>
          {[
            { label: 'Toplam Personel', val: kullanicilar?.length ?? 0, icon: '👥' },
            { label: 'Online (son 2dk)', val: (kullanicilar ?? []).filter((u: any) => u.last_seen_at && u.last_seen_at > onlineSince).length, icon: '🟢' },
        ...
        <div className="verde-card" style={{ overflow: 'hidden' }}>
          <div style={{ padding: '14px 18px', borderBottom: '1px solid #e8f0e8', fontSize: 14, fontWeight: 800 }}>
            Personel — {aktifProje ? aktifProje.ad : 'Tüm Firma'}
          </div>
          {(kullanicilar ?? []).length === 0 ? (
            <div style={{ padding: 40, textAlign: 'center', color: '#7a907a', fontSize: 14 }}>Personel bulunamadı.</div>
          ) : (
            <table className="verde-table">
              <thead>
                <tr><th>Personel</th><th>Grup</th><th>Durum</th><th>Son 7 Gün</th><th>Tamamlanan</th><th>Kayıp</th><th>Başarı</th></tr>
              </thead>
              <tbody>
                {(kullanicilar ?? []).map((u: any) => {
                  ...
                  const rolBadge: Record<string, { bg: string; color: string; kisa: string }> = {
                  ...
                  return (
                    <tr key={u.id}>
                      ...
                      <td>{rb && <span style={{ fontSize: 11.5, fontWeight: 800, padding: '2px 8px', borderRadius: 6, background: rb.bg, color: rb.color }}>{rb.kisa}</span>}</td>
                      <td>
                      ...
                      <td>
                        {basari !== null ? (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            <span style={{ fontSize: 12, fontWeight: 700, minWidth: 36 }}>%{basari}</span>
                          </div>
