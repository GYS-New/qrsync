...
export async function POST(req: Request, { params }: { params: { token: string } }) {
  ...
  try {
    ...
    // Max süre: MANUEL tamamlama serbest — sorumlu personel niyetli, kabul edilir.
    // Sadece cron (/api/tasks/max-sure-kontrol) unutulan görevleri IPTAL'e çeker
    // (sebep='Görev Zaman Aşımı'). Bu ayrım kullanıcı niyetini yansıtır.
