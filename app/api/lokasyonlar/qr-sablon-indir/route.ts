...
import { fillQrKartWithPython, type QrKartAyarlar } from '@/lib/qr-kart/fill-qr-kart'

export const runtime = 'nodejs'
export const maxDuration = 120

/**
 * POST /api/lokasyonlar/qr-sablon-indir
 * FormData:
 *   sablon      — PNG/JPG şablon dosyası
 *   firma_id    — string
 *   proje_id    — string (opsiyonel)
 *   origin      — string (opsiyonel, QR URL base)
 *   ayarlar     — JSON string (opsiyonel koordinat overrides)
 */
export async function POST(req: NextRequest) {
  ...
  // Alt torun id'lerini özyinelemeli topla
  ...
  const zipBuffer = await fillQrKartWithPython(sablonBuffer, payload, sablonExt)
