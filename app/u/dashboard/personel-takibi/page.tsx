...
export default async function UPersonelTakibiPage() {
  ...
  // Projeye bağlı kullanıcılar — projeId yoksa firma geneli
  let kulQ = admin
    .from('users')
  ...
  // Son 7 günlük görev istatistikleri
  ...
  return (
    <div>
      <Topbar
        ...
        base="/u"
        breadcrumbs={[{ label: 'Yönetim' }, { label: 'Personel Takibi' }]}
      />
        ...
        <div className="verde-card" style={{ overflow: 'hidden' }}>
          ...
          {(kullanicilar ?? []).length === 0 ? (
            <div style={{ padding: 40, textAlign: 'center', color: '#7a907a', fontSize: 14 }}>
              Bu projede henüz personel bulunamadı.
            </div>
          ) : (
