-- 097: PG vardiya fonksiyonlarında "bitis=00:00 = gün sonu" özel durum fix
--
-- Bug: Çanakkale projesi V3 = "16:00 - 00:00" (akşam vardiyası, gün sonunda biten).
-- vardiya_gunu_hesapla ve gece_gorev_uret string karşılaştırma ile sarkan algılıyordu:
--   v_bit "00:00" <= v_bas "16:00" → TRUE → sarkan zannedildi
-- Doğru: "00:00" bitiş "24:00" = gün sonu anlamında, sarkan değil.
--
-- Sonuçlar (bug süresince):
--   - vardiya_gunu_hesapla 23:35'te yapılan görevi YARIN'a atıyordu (yanlış)
--   - gece_gorev_uret 16:30'da aktif olacak Çanakkale kuralını ÖNCEKİ GÜNE üretiyordu
--     (v_gun_offset = -1) → kural ertesi gün rapora değil önceki gün rapora denk geliyordu
--
-- Etkilenenler:
--   - Sadece "bitis=00:00, baslangic!=00:00" formundaki vardiyalar
--   - DB'de şu an: ATALIAN/Çanakkale V3 (16:00-00:00) — tek vaka
--   - OYAK RENAULT (sarkan V1 23:30-07:30) etkilenmez — gerçek sarkan
--   - APPLE V3 (16:00-23:59) etkilenmez — "00:00" yerine "23:59" yazılmış (workaround)
--
-- Fix: v_bit = '00:00' AND v_bas <> '00:00' ise v_bit_eff := '24:00' normalize.
-- Sonra mevcut "<=" karşılaştırması doğru çalışır (24:00 > 16:00 → sarkan değil).

-- ─── 1) vardiya_gunu_hesapla fix ────────────────────────────────────────
...
BEGIN
  ...
  FOR k IN 0..(jsonb_array_length(v_aktif_set) - 1) LOOP
    ...
    -- '00:00' bitiş = gün sonu (24:00 anlamında); sarkan değil
    v_bit_eff := CASE WHEN v_bit = '00:00' AND v_bas <> '00:00' THEN '24:00' ELSE v_bit END;
    -- Sarkan vardiya (örn V1 23:30-07:30) + saat evening half'ta → yarın
    IF v_bit_eff <= v_bas AND v_tr_saat >= v_bas THEN
      RETURN v_tr_date + 1;
    END IF;
  END LOOP;
  RETURN v_tr_date;
...
-- ─── 2) gece_gorev_uret fix (mig 095'in son halini güncelle) ─────────────
-- Sadece v_bit / v_bas karşılaştırmasında v_bit_eff kullanılır;
-- diğer mantık aynı kalır.
