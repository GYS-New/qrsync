  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      ...
      <div className="verde-card" style={{ padding: '16px 20px', marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <div style={{ fontSize: 16, fontWeight: 900, color: '#111827' }}>LOKASYON GRUPLARI</div>
        ...
        <button
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 8, border: '1px solid #e5e7eb', background: '#f8fbf8', cursor: 'pointer', fontSize: 13, color: '#4b5563', fontWeight: 600 }}
        >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: readonly ? '1fr' : 'minmax(320px, 380px) 1fr', gap: 20, alignItems: 'start' }}>
        ...
        <div className="verde-card" style={{ padding: '18px 20px', position: 'sticky', top: 88 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 28, height: 28, borderRadius: 7, background: '#e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {editingId ? <Pencil size={13} color="#1f2937" /> : <Plus size={13} color="#1f2937" />}
              </div>
              <span style={{ fontSize: 14, fontWeight: 800, color: '#111827' }}>{editingId ? 'Grubu Düzenle' : 'Yeni Grup Oluştur'}</span>
            </div>
            {editingId && (
              <button onClick={resetForm} style={{ fontSize: 12, color: '#6b7280', border: 'none', background: 'none', cursor: 'pointer', padding: '4px 8px' }}>İptal</button>
            )}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11.5, color: '#6b7280', marginTop: 4 }}>Grup üyeleri sadece seçilen üst lokasyonun altından seçilebilir.</div>
            </div>
            ...
            {/* Kapsam özeti */}
            <div style={{ padding: '10px 12px', borderRadius: 8, background: '#f9fafb', border: '1px solid #e5e7eb', fontSize: 13 }}>
              <div style={{ fontWeight: 700, color: '#1f2937', marginBottom: 2 }}>Seçili Kapsam</div>
              <div style={{ color: '#4b5563' }}>{selectedTopPath || 'En üst lokasyon seçilmedi'}</div>
              <div style={{ color: '#6b7280', marginTop: 2 }}>{selectedLocationIds.length} adet alt lokasyon bu gruba bağlı.</div>
            </div>
            ...
            <div>
              ...
              <div style={{ border: '1px solid #e0ece0', borderRadius: 8, overflow: 'hidden', maxHeight: 300, overflowY: 'auto' }}>
                {!topLocationId ? (
                  <div style={{ padding: '12px 14px', color: '#6b7280', fontSize: 13 }}>Önce en üst lokasyon seçin.</div>
                ) : locationOptions.length === 0 ? (
                  <div style={{ padding: '12px 14px', color: '#6b7280', fontSize: 13 }}>Bu kapsamda lokasyon bulunamadı.</div>
                ) : locationOptions.map((loc) => {
                  const checked = selectedLocationIds.includes(loc.id)
                  return (
                    <label key={loc.id} style={{ display: 'flex', gap: 10, padding: '9px 12px', borderBottom: '1px solid #f0f4f0', cursor: 'pointer', alignItems: 'flex-start', background: checked ? '#f9fafb' : '#fff' }}>
                      <input type="checkbox" checked={checked} onChange={() => setSelectedLocationIds((prev) => checked ? prev.filter((x) => x !== loc.id) : [...prev, loc.id])} style={{ marginTop: 2, accentColor: '#374151' }} />
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontWeight: 600, color: '#111827', fontSize: 13 }}>{loc.tanim}</div>
                        <div style={{ color: '#6b7280', fontSize: 12, marginTop: 1, wordBreak: 'break-word' }}>{loc.path}</div>
                      </div>
                    </label>
                  )
                })}
            ...
            <button
              ...
              style={{ padding: '10px', borderRadius: 8, border: 'none', background: saving ? '#9ca3af' : '#374151', color: '#fff', fontWeight: 700, fontSize: 14, cursor: saving || !firmaId ? 'not-allowed' : 'pointer' }}
            >
        ...
        <div style={{ position: 'sticky', top: 88, maxHeight: 'calc(100vh - 120px)', display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, flexShrink: 0 }}>
            <div style={{ fontSize: 14, fontWeight: 800, color: '#111827' }}>Mevcut Gruplar</div>
            <div style={{ fontSize: 12.5, color: '#6b7280' }}>{groupsFlat.length} grup · {grouped.length} üst lokasyon</div>
          </div>
          <div style={{ overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 10, flex: 1, paddingRight: 4 }}>

          {grouped.length === 0 ? (
            <div className="verde-card" style={{ padding: 32, textAlign: 'center', color: '#6b7280' }}>
              <Layers size={32} style={{ margin: '0 auto 10px', color: '#d1d5db' }} />
              ...
              {(() => {
                ...
                return (
                  <>
                    <div
                      ...
                      style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 8, cursor: 'pointer', background: sectionExpanded ? '#f9fafb' : '#f4f8f4', border: `1px solid ${sectionExpanded ? '#d1d5db' : '#f3f4f6'}`, transition: 'all .15s' }}
                    >
                      <div style={{ width: 28, height: 28, borderRadius: 7, background: '#111827', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <Layers size={13} color="#fff" />
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13, fontWeight: 900, color: '#111827', letterSpacing: 0.2 }}>
                          {section.topTanim}
                        </div>
                        {section.topPath !== section.topTanim && (
                          <div style={{ fontSize: 11, color: '#6b7280' }}>{section.topPath}</div>
                        )}
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontSize: 11.5, color: '#6b7280', fontWeight: 600 }}>{section.items.length} grup</span>
                        {sectionExpanded
                ...
                return (
                  <div key={group.id} className="verde-card"
                    style={{ overflow: 'hidden', marginLeft: 20, border: isEditing ? '1.5px solid #374151' : '1px solid #f3f4f6' }}>
                    {/* Grup başlığı */}
                    <div style={{ padding: '11px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, cursor: 'pointer' }} onClick={() => toggleExpand(group.id)}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 0 }}>
                        <div style={{ width: 30, height: 30, borderRadius: 7, background: isEditing ? '#e5e7eb' : '#f4f8f4', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <Layers size={13} color={isEditing ? '#1f2937' : '#6b7280'} />
                        </div>
                        <div style={{ minWidth: 0 }}>
                          <div style={{ fontSize: 13.5, fontWeight: 800, color: '#111827', display: 'flex', alignItems: 'center', gap: 7 }}>
                            {group.ad}
                            {isEditing && <span style={{ fontSize: 10, fontWeight: 700, padding: '1px 6px', borderRadius: 4, background: '#e5e7eb', color: '#1f2937' }}>Düzenleniyor</span>}
                          </div>
                          <div style={{ fontSize: 11.5, color: '#6b7280', marginTop: 1 }}>
                            <span style={{ fontWeight: 600, color: '#374151' }}>{group.memberPaths.length} lokasyon</span>
                      ...
                      <div style={{ borderTop: '1px solid #f3f4f6', padding: '10px 14px' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 6 }}>
                          {group.memberPaths.length === 0 ? (
                            <div style={{ fontSize: 13, color: '#6b7280' }}>Bu gruba henüz lokasyon eklenmemiş.</div>
