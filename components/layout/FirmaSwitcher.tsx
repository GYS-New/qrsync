...
export default function FirmaSwitcher() {
  ...
  if (loading) return (
    <div style={{ margin: '6px 10px', padding: '8px 10px', borderRadius: 8, background: '#f0f9f0', border: '1px solid #e0ece0' }}>
      <div style={{ fontSize: 12, color: '#7a907a' }}>Firmalar yükleniyor…</div>
    </div>
  )
    ...
    <div ref={ref} style={{ margin: '6px 10px', position: 'relative' }}>
      <button
        ...
        style={{
          ...
          background: selectedFirma ? '#e8f4ff' : '#f0f9f0',
          border: `1px solid ${selectedFirma ? '#b0d4f0' : '#d6e4d6'}`,
          cursor: 'pointer', textAlign: 'left',
        }}
      >
        <Building2 size={14} style={{ color: selectedFirma ? '#185a9b' : '#2e8b2e', flexShrink: 0 }} />
        <span style={{
          flex: 1, fontSize: 12.5, fontWeight: 700,
          color: selectedFirma ? '#185a9b' : '#1f6b1f',
          overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
        }}>
          {label}
        </span>
        <ChevronDown size={13} style={{
          color: '#7a907a', flexShrink: 0,
      ...
      {open && (
        <div style={{
          ...
          <button
            ...
            style={{
              width: '100%', display: 'flex', alignItems: 'center', gap: 8,
              padding: '9px 12px', background: !selectedFirma ? '#f0f9f0' : 'transparent',
              border: 'none', cursor: 'pointer', textAlign: 'left',
              borderBottom: '1px solid #f0f4f0',
            }}
            onMouseEnter={e => { if (selectedFirma) (e.currentTarget as HTMLElement).style.background = '#f7fbf7' }}
            onMouseLeave={e => { if (selectedFirma) (e.currentTarget as HTMLElement).style.background = 'transparent' }}
          >
            <Building2 size={13} style={{ color: '#7a907a', flexShrink: 0 }} />
            <span style={{ fontSize: 12.5, color: !selectedFirma ? '#1f6b1f' : '#2b3a2b', fontWeight: !selectedFirma ? 700 : 400 }}>
              Tüm Firmalar
            </span>
            {!selectedFirma && <span style={{ marginLeft: 'auto', fontSize: 10, color: '#2e8b2e', fontWeight: 700 }}>✓</span>}
          </button>
          ...
          {firmalar.map(f => {
            ...
            return (
              <button
                ...
                onMouseEnter={e => { if (!isSelected) (e.currentTarget as HTMLElement).style.background = '#f7fbf7' }}
                onMouseLeave={e => { if (!isSelected) (e.currentTarget as HTMLElement).style.background = 'transparent' }}
              >
                <Building2 size={13} style={{ color: isSelected ? '#185a9b' : '#7a907a', flexShrink: 0 }} />
                <span style={{
