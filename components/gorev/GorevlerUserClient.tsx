      ...
      <div className="verde-card">
        <div style={{ padding: '14px 18px', borderBottom: '1px solid #e8f0e8', display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
            <span style={{ fontSize: 12.5, color: '#7a907a' }}>
              Toplam <strong style={{ color: '#2e8b2e' }}>{filtered.length}</strong> görev
            </span>
        ...
        <table className="verde-table">
          ...
          <tbody>
                ...
                <tr key={g.id} style={mine ? {} : { opacity: 0.7 }}>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
                      ...
                      {mine && g.durum === 'ACIK' && (
                        <span style={{ fontSize: 11.5, color: '#7a907a' }}>Bildirimden kabul/ret</span>
                      )}
                      {!mine && (
            ...
            {!filtered.length && (
              <tr>
                <td colSpan={6} style={{ textAlign: 'center', color: '#7a907a', padding: '36px 0' }}>
                  Görev bulunamadı
                </td>
              </tr>
            )}
