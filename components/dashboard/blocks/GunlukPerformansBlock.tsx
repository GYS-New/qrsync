...
const STAT = [
  { key: 'TAMAMLANDI',           label: 'Tamamlandı', color: '#ff7f00', bg: '#ffe4bc' },
  { key: 'ISLEMDE',              label: 'İşlemde',    color: '#c2610c', bg: '#fff7ed' },
  ...
  return (
    <div className="verde-card h-[420px] flex flex-col">
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#3d1c00' }}>GÜNLÜK PERFORMANS</div>
          <div style={{ fontSize: 12, color: '#9a7b6a', marginTop: 2 }}>Spesifik görevler — bugün</div>
        </div>
      ...
      <div className="flex-1 flex flex-col" style={{ padding: '14px 18px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
          <div style={{ fontSize: 14, fontWeight: 800, color: '#5c3a1e' }}>Toplam</div>
          <div style={{ fontSize: 24, fontWeight: 900, color: '#3d1c00' }}>{total}</div>
        </div>

        <div className="flex-1" style={{ marginTop: 12, minHeight: 0 }}>
          <div style={{
            height: '100%', border: '1px solid #ffe8c8', borderRadius: 12,
            padding: '14px 14px 12px',
            background: 'linear-gradient(180deg, rgba(240,249,240,.85), rgba(255,255,255,.92))',
            display: 'flex', flexDirection: 'column',
          }}>
            <div style={{ fontSize: 12.5, color: '#9a7b6a', fontWeight: 700, letterSpacing: 0.6, textTransform: 'uppercase' }}>
              Durum Dağılımı
            </div>
            <div style={{
              ...
              {STAT.map((s) => {
                ...
                return (
                  <div key={s.key} style={{ display: 'flex', flexDirection: 'column', height: '100%', minWidth: 0 }}>
                    <div style={{ fontSize: 16, fontWeight: 900, color: '#3d1c00', textAlign: 'center', lineHeight: 1.1 }}>
                      {v}
                    </div>
                    ...
                    <div style={{
                      fontSize: 12.5, color: '#5c3a1e', fontWeight: 900, textAlign: 'center',
        ...
        {total === 0 && (
          <div style={{ textAlign: 'center', color: '#9a7b6a', paddingTop: 12, fontSize: 13.5 }}>
