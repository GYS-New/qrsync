...
const STAT = [
  { key: 'TAMAMLANDI',           label: 'Tamamlandı', color: '#2e8b2e', bg: '#dcf0dc' },
  ...
  return (
    <div className="verde-card h-[420px] flex flex-col">
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#0f1a0f' }}>GÜNLÜK PERFORMANS</div>
          <div style={{ fontSize: 12, color: '#7a907a', marginTop: 2 }}>Spesifik görevler — bugün</div>
        </div>
      ...
      <div className="flex-1 flex flex-col" style={{ padding: '14px 18px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
          <div style={{ fontSize: 14, fontWeight: 800, color: '#2d3f2d' }}>Toplam</div>
          <div style={{ fontSize: 24, fontWeight: 900, color: '#0f1a0f' }}>{total}</div>
        </div>

        <div className="flex-1" style={{ marginTop: 12, minHeight: 0 }}>
          <div style={{
            height: '100%', border: '1px solid #e8f0e8', borderRadius: 12,
            padding: '14px 14px 12px',
            background: 'linear-gradient(180deg, rgba(240,249,240,.85), rgba(255,255,255,.92))',
            display: 'flex', flexDirection: 'column',
          }}>
            <div style={{ fontSize: 12.5, color: '#7a907a', fontWeight: 700, letterSpacing: 0.6, textTransform: 'uppercase' }}>
              Durum Dağılımı
            </div>
            <div style={{
              ...
              {STAT.map((s) => {
                ...
                return (
                  <div key={s.key} style={{ display: 'flex', flexDirection: 'column', height: '100%', minWidth: 0 }}>
                    <div style={{ fontSize: 16, fontWeight: 900, color: '#0f1a0f', textAlign: 'center', lineHeight: 1.1 }}>
                      {v}
                    </div>
                    ...
                    <div style={{
                      fontSize: 12.5, color: '#2d3f2d', fontWeight: 900, textAlign: 'center',
