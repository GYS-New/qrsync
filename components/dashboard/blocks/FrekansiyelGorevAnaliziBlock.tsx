...
export default function FrekansiyelGorevAnaliziBlock({ firmaId, projeId, basePath }: DashboardBlockProps & { firmaId: string | null; projeId?: string | null }) {
  ...
  // Measure container width once and render charts only when width is known.
  ...
  const scopeLabel = useMemo(() => (firmaId ? '' : ''), [firmaId])
  ...
  async function fetchCounts() {
    ...
    const base = (q: any) => firmaFilter(q).gte('olusturma_tarihi', rangeStart.toISOString())
    ...
    const [
      ...
      // Total: exclude IPTAL to keep charts meaningful
      base(supabase.from('canli_gorevler').select('*', { count: 'exact', head: true }).neq('durum', 'IPTAL')),
      base(supabase.from('canli_gorevler').select('*', { count: 'exact', head: true }).eq('durum', 'TAMAMLANDI')),
      base(supabase.from('canli_gorevler').select('*', { count: 'exact', head: true }).eq('durum', 'ZAMANINDA_YAPILAMAYAN')),
      base(supabase.from('canli_gorevler').select('*', { count: 'exact', head: true }).eq('durum', 'BEKLEMEDE')),
    ])
  ...
  return (
    <BlockWrapper
      title="FREKANSİYEL GÖREV ANALİZİ"
      size="big"
      
      href={`${basePath}/dashboard/canli-islemler`}
    >
      <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
        <div style={{ fontSize: 13, color: '#7a907a' }}>{scopeLabel} • {mode === 'gunluk' ? 'Son 24 saat' : mode === 'haftalik' ? 'Son 7 gün' : 'Son 30 gün'}</div>
      ...
      <div ref={wrapRef} style={{ width: '100%', minWidth: 0 }}>
        {loading ? <div style={{ fontSize: 12, color: '#7a907a', marginBottom: 10 }}>Yükleniyor...</div> : null}

        {/*
          Layout note:
          Keep all 3 charts equal width on lg+ screens to prevent horizontal overflow
          and match the requested layout.
        */}
