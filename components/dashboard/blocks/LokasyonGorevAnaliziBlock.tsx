...
export default function LokasyonGorevAnaliziBlock({
  ...
  async function fetchData() {
    ...
    try {
      ...
      const sel = `lokasyon_id,olusturma_tarihi,lokasyonlar(tanim,parent_id,parent:parent_id(tanim))`
    ...
    <div className="verde-card h-[420px] flex flex-col">
      <div style={{ padding: '16px 18px 12px', borderBottom: '1px solid #e8f0e8', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#0f1a0f' }}>LOKASYON GÖREV ANALİZİ</div>
        ...
        <Link href={`${basePath}/dashboard/lokasyonlar`} className="text-[13px] text-[#2e8b2e] hover:underline mt-[2px]">Tümünü Gör</Link>
      </div>
      ...
      <div className="flex-1 flex flex-col" style={{ padding: '12px 18px 16px' }}>
              ...
              return (
                <div key={i} style={{ padding: '12px 14px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '44px 1fr 54px', gap: 10, alignItems: 'center' }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      {r?.parentName && <div style={{ fontSize: 11.5, fontWeight: 900, color: '#7a907a', textTransform: 'uppercase', letterSpacing: 0.6, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', opacity: isEmpty ? 0.45 : 1 }}>{r.parentName}</div>}
                      <div style={{ fontSize: 13.5, fontWeight: 800, color: '#0f1a0f', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', opacity: isEmpty ? 0.45 : 1 }}>{isEmpty ? '—' : r.name}</div>
        ...
        {rows.length === 0 && !loading && (
          <div style={{ textAlign: 'center', color: '#7a907a', paddingTop: 12, fontSize: 13.5 }}>Seçilen aralıkta frekansiyel görev bulunamadı</div>
        )}
      </div>
    </div>
  )
