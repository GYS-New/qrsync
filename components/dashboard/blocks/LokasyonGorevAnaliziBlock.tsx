...
export default function LokasyonGorevAnaliziBlock({ firmaId, projeId,
  basePath,
  ...
  async function fetchData() {
    ...
    try {
      let q = supabase
        .from('canli_gorevler')
        .select(
          `lokasyon_id,olusturma_tarihi,
           lokasyonlar(tanim,parent_id,parent:parent_id(tanim))`
        )
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('dashboard-lokasyon-gorev')
    ...
    <div className="verde-card h-[420px] flex flex-col">
      ...
      >
        <div style={{ minWidth: 0 }}>
          ...
          >
            En çok göreve sahip 3 lokasyon
          </div>
        </div>

        <Link href={`${basePath}/dashboard/lokasyonlar`} className="text-[13px] text-[#2e8b2e] hover:underline mt-[2px]">
          Tümünü Gör
        </Link>
      </div>
      ...
      <div className="flex-1 flex flex-col" style={{ padding: '12px 18px 16px' }}>
        ...
        {rows.length === 0 && !loading && (
          <div style={{ textAlign: 'center', color: '#7a907a', paddingTop: 12, fontSize: 13.5 }}>
            Seçilen aralıkta frekansiyel görev bulunamadı
          </div>
        )}
      </div>
    </div>
