...
export default function GorevSureleriClient({ lokasyonlar }: Props) {
  ...
  // Açık accordion öğeleri
  const [openIds, setOpenIds] = useState<Set<string>>(new Set())
  // Draft değerleri: { [lokId]: { min: string, max: string } }
      ...
      <div key={lok.id}>
        ...
        <div
          style={{
            ...
            background: indent === 0 ? '#f8faf8' : '#fff',
            minHeight: 48,
          }}
        >
          {/* Accordion toggle (sadece alt lokasyonu olanlar için) */}
          {hasChildren ? (
            <button
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
            ...
            <input
              ...
              min={0}
              value={d.min}
              onChange={e => setDraft(lok.id, 'min', e.target.value)}
              placeholder="—"
              style={inputStyle}
            />
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
            ...
            <input
              ...
              min={0}
              value={d.max}
              onChange={e => setDraft(lok.id, 'max', e.target.value)}
              placeholder="—"
              style={inputStyle}
            />
          </div>
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <br />
        <span style={{ color: '#6b7280' }}>Değer girilmezse (boş bırakılırsa) ilgili kural uygulanmaz.</span>
      </div>
