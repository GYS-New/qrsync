...
export default function ProjeSwitcher() {
  ...
  const label = aktifProje?.ad ?? 'Proje yükleniyor…'
  // Tek aktif proje varsa dropdown gerekmez — statik gösterim
  const tekProje = projeler.length === 1

  return (
    <div ref={ref} style={{ margin: '6px 10px', position: 'relative' }}>
      {/* Seçili proje butonu */}
      <button
        onClick={() => { if (!tekProje) setOpen(v => !v) }}
        disabled={tekProje}
        style={{
          ...
          cursor: tekProje ? 'default' : 'pointer', textAlign: 'left',
          opacity: tekProje ? 0.95 : 1,
        }}
      >
      ...
      {/* Dropdown — sadece çoklu proje varsa */}
      {open && !tekProje && (
        <div style={{
          ...
          {/* Proje listesi — "Tüm Projeler" seçeneği kaldırıldı (2026-05-08) */}
