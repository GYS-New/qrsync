...
const palette = [
  {
    type: 'kpi' as const,
    title: 'KPI Kartı',
    accent: 'linear-gradient(135deg, #4caf68 0%, #2e8b57 100%)',
      ...
      <thead>
        <tr>
          {preview.columns.map((column) => (
            <th key={column.key} style={{ textAlign: 'left', padding: '8px 10px', borderBottom: '1px solid #dfe8df', color: '#536553', fontWeight: 900, whiteSpace: 'nowrap' }}>
              {column.label}
            </th>
          ))}
        </tr>
      ...
      <div style={{ padding: 20, display: 'grid', gap: 16 }}>
        <section className="verde-card" style={{ padding: 16, borderRadius: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <button type="button" onClick={() => loadPreset('blank')} style={{ padding: '10px 14px', borderRadius: 14, border: mode === 'blank' ? '1px solid #5c49db' : '1px solid #d7e2d7', background: mode === 'blank' ? '#f4f1ff' : '#fff', color: mode === 'blank' ? '#5c49db' : '#2d3f2d', fontWeight: 800, cursor: 'pointer' }}>Boş Tuval</button>
              <button type="button" onClick={() => loadPreset('general')} style={{ padding: '10px 14px', borderRadius: 14, border: mode === 'general' ? '1px solid #2e8b57' : '1px solid #d7e2d7', background: mode === 'general' ? '#eff8ef' : '#fff', color: mode === 'general' ? '#2e8b57' : '#2d3f2d', fontWeight: 800, cursor: 'pointer' }}>Genel Şablon</button>
              <button type="button" onClick={() => loadPreset(mode)} style={{ padding: '10px 14px', borderRadius: 14, border: '1px solid #d7e2d7', background: '#fff', color: '#2d3f2d', fontWeight: 800, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8 }}><RefreshCcw size={15} />Sıfırla</button>
              <button type="button" style={{ padding: '10px 14px', borderRadius: 14, border: '1px solid #d7e2d7', background: '#fff', color: '#2d3f2d', fontWeight: 800, display: 'inline-flex', alignItems: 'center', gap: 8 }}><Save size={15} />Taslağı Kaydet</button>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, minmax(0, 1fr))', gap: 12 }}>
            {palette.map((item) => (
              <button
                ...
                style={{ borderRadius: 18, border: '1px solid #dfe8df', background: '#fff', padding: 14, display: 'grid', gap: 10, textAlign: 'left', cursor: 'pointer', minHeight: 94 }}
              >
        ...
        <section className="verde-card" style={{ padding: 16, borderRadius: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <div style={{ padding: '9px 12px', borderRadius: 999, border: '1px solid #dfe8df', background: '#fbfdfb', color: '#425242', fontSize: 12.5, fontWeight: 800 }}>Tut-çek: başlıktan taşı</div>
              <div style={{ padding: '9px 12px', borderRadius: 999, border: '1px solid #dfe8df', background: '#fbfdfb', color: '#425242', fontSize: 12.5, fontWeight: 800 }}>Sağ alttan boyutlandır</div>
            </div>
          </div>
          ...
          <div ref={viewportRef} style={{ width: '100%', minHeight: 'calc(100vh - 360px)', overflow: 'auto', padding: 8, background: '#f6f9f6', borderRadius: 20 }}>
            <div style={{ width: PAGE_WIDTH * canvasScale, height: canvasHeight * canvasScale, position: 'relative', margin: '0 auto' }}>
              <div style={{ position: 'absolute', left: 0, top: 0, width: PAGE_WIDTH, height: canvasHeight, transform: `scale(${canvasScale})`, transformOrigin: 'top left' }}>
                <div style={{ position: 'absolute', inset: 0, borderRadius: 28, background: '#fff', border: '1px solid #dfe8df', boxShadow: '0 20px 40px rgba(15,26,15,0.08)' }} />
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        height,
                        borderRadius: 18,
                        border: selectedWidgetId === widget.id ? '2px solid #5c49db' : '1px solid #dfe8df',
                        background: '#fff',
                        padding: 14,
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                        ...
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                          {widget.type === 'table' ? <Database size={15} color="#5d6d5d" /> : <Grip size={15} color="#5d6d5d" />}
                          {widgets.length > 1 ? (
                      ...
                      <span onMouseDown={(event) => startResize(event, widget.id)} style={{ position: 'absolute', right: 10, bottom: 10, width: 18, height: 18, borderRadius: 6, border: '1px solid #dfe8df', background: '#fff', display: 'grid', placeItems: 'center', color: '#6a7b6a', cursor: 'nwse-resize' }}>
                        <LayoutGrid size={10} />
                      </span>
                    </div>
                  )
        ...
        <section className="verde-card" style={{ padding: 16, borderRadius: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: selected?.type === 'table' ? '1.1fr 1fr 1.4fr' : '1fr 1fr', gap: 14 }}>
            <div style={{ border: '1px solid #dfe8df', borderRadius: 18, background: '#fff', padding: 14 }}>
                ...
                <div style={{ border: '1px solid #dfe8df', borderRadius: 18, background: '#fff', padding: 14 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 8 }}>
                    {selectedTableDef.columns.map((column) => {
                      ...
                      return (
                        <label key={column.key} style={{ display: 'flex', alignItems: 'center', gap: 8, border: checked ? '1px solid #2e8b57' : '1px solid #dfe8df', borderRadius: 12, background: checked ? '#eff8ef' : '#fff', padding: '8px 10px', cursor: 'pointer' }}>
                          <input
                            type="checkbox"
                            checked={!!checked}
                            onChange={(e) => {
        ...
        <section className="verde-card" style={{ padding: 20, borderRadius: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
            ...
            <a href={`${base}/dashboard/raporlar/grafiksel`} style={{ padding: '12px 16px', borderRadius: 16, border: '1px solid #dfe8df', background: '#fff', color: '#2d3f2d', fontWeight: 800, display: 'inline-flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
