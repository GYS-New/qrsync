  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      <div className="verde-card">
        ...
        {!firmaId && base === '/sa' ? (
          <div style={{ padding: '48px', textAlign: 'center', color: '#7a907a' }}>Checklistleri görmek için firma seçin.</div>
        ) : (
          <div style={{ padding: 18, display: 'grid', gap: 12 }}>
            {!rows.length ? (
              <div style={{ color: '#7a907a' }}>Bu firmada henüz lokasyon veya checklist bulunamadı.</div>
            ) : rows.map(({ lokasyon, template }) => (
              <div key={lokasyon.id} style={{ border: '1px solid #d6e4d6', borderRadius: 12, padding: 14, background: '#fff' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                  <div>
                    <div style={{ fontWeight: 800, color: '#0f1a0f' }}>{lokasyon.tanim}</div>
                    <div style={{ fontSize: 12, color: '#7a907a', marginTop: 4 }}>{template ? `Checklist: ${template.isim}` : 'Checklist bağlı değil'}</div>
                    {template?.checklist_items?.length ? (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setOpenForm(false)}>
          <div className="verde-card" style={{ width: 760, maxWidth: 'calc(100vw - 32px)', padding: 0, overflow: 'hidden' }} onClick={(e) => e.stopPropagation()}>
            <div style={{ padding: '16px 18px', borderBottom: '1px solid #e8f0e8', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#0f1a0f' }}>{editing ? 'Checklist Düzenle' : 'Checklist Ekle'}</div>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gap: 10 }}>
                  {items.map((item, index) => (
                    <div key={item.id || index} style={{ border: '1px solid #d6e4d6', borderRadius: 10, padding: 12, display: 'grid', gap: 10 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center' }}>
                        <div style={{ fontSize: 12, color: '#7a907a' }}>Madde #{index + 1}</div>
