      ...
      <div key={grup.id} style={{ background: '#fff', borderTop: '1px solid #eaf3ea' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 14px 9px 32px' }}>
          ...
          <span style={{ flex: 1, fontSize: 13, fontWeight: 700, color: '#1a3a1a' }}>
            🗂 {grup.ad}
            <span style={{ fontSize: 11, color: '#7a907a', fontWeight: 400, marginLeft: 6 }}>
          ...
          <input
            ...
            placeholder="0.00"
            style={{ width: 100, padding: '4px 8px', borderRadius: 6, fontSize: 13, border: '1px solid #d6e4d6', background: readonly ? '#f5f5f5' : '#fff', color: '#1a3a1a' }}
          />
          <select
            disabled={readonly}
            value={grupDraft.para_birimi}
            onChange={e => onGrupPbChange(grup.id, e.target.value)}
            style={{ padding: '4px 6px', borderRadius: 6, border: '1px solid #d6e4d6', fontSize: 12, background: readonly ? '#f5f5f5' : '#fff' }}
          >
          ...
          {!readonly && (
            <button
              ...
              style={{ padding: '4px 14px', borderRadius: 6, fontSize: 12, fontWeight: 700, border: '1px solid #86efac', background: isSaving ? '#f0f9f0' : '#dcfce7', color: '#15803d', cursor: isSaving ? 'default' : 'pointer', whiteSpace: 'nowrap' }}
            >
            ...
            {memberLoks.map((lok, idx) => {
              ...
              return (
                <div key={lok.id} style={{
                  ...
                  <span style={{ flex: 1, fontSize: 12, color: '#506050', display: 'flex', alignItems: 'center', gap: 5 }}>
                  ...
                  <input
                    ...
                    placeholder="0.00"
                    style={{ width: 90, padding: '3px 7px', borderRadius: 6, fontSize: 12, border: '1px solid #d6e4d6', background: readonly ? '#f5f5f5' : '#fff', color: '#1a3a1a' }}
                  />
                  <select
                    disabled={readonly}
                    value={lokDraft.para_birimi}
                    onChange={e => setDrafts(prev => ({ ...prev, [lokKey]: { ...getDraft(lokKey), para_birimi: e.target.value } }))}
                    style={{ padding: '3px 5px', borderRadius: 6, border: '1px solid #d6e4d6', fontSize: 11, background: readonly ? '#f5f5f5' : '#fff' }}
                  >
                    {PARA_BIRIMLERI.map(p => <option key={p} value={p}>{p}</option>)}
                  </select>
                  {!readonly && (
                    <button
                      ...
                      style={{ padding: '3px 10px', borderRadius: 6, fontSize: 11, fontWeight: 600, border: '1px solid #86efac', background: isLokSaving ? '#f0f9f0' : '#dcfce7', color: '#15803d', cursor: isLokSaving ? 'default' : 'pointer' }}
                    >
                      {isLokSaving ? '…' : 'Kaydet'}
                    </button>
                  )}
          ...
          <div key={ustLok.id} style={{ border: '1px solid #c8dcc8', borderRadius: 10, overflow: 'hidden' }}>
            ...
            >
              <span style={{ fontSize: 14, fontWeight: 800, color: '#1a3a1a', flex: 1 }}>
                📍 {ustLok.tanim}
                <span style={{ fontSize: 11, color: '#7a907a', fontWeight: 400, marginLeft: 8 }}>
