...
function initArsivCron() {
  ...
  cron.schedule('*/5 * * * *', async () => {
    try {
      ...
      await logCron('max_sure', { heartbeat: true, ok: result.ok ?? true, gorevler_iptal: result.gorevler_iptal ?? 0, canli_gorevler_iptal: result.canli_gorevler_iptal ?? 0 })
  ...
  // Vardiya bitişine 30 dk kala: 07:30, 15:30, 23:30 TRT → 04:30, 12:30, 20:30 UTC
  // Bitmek üzere olan vardiyanın tamamlanmamış görevlerini hedef % oranında kapatır.
  cron.schedule('30 4,12,20 * * *', async () => {
    try {
  ...
  console.log('[CRON] Personel görev desteği cron başlatıldı (vardiya bitişine 30 dk kala: 07:30, 15:30, 23:30 TRT)')
