...
function initArsivCron() {
  ...
  // Her 15 dk: PD calistir endpoint'i. İçeride her aktif destek kaydı için
  // o projenin efektif vardiyasının bitişi 30-45 dk önce mi diye kontrol edilir.
  // Pencerede olmayan kayıt skip (skipReason='penceredeDegil').
  //
  // GEÇMİŞ: 0 21,5,13 * * * (00:00/08:00/16:00 TRT) → Renault'a hardcoded.
  // Çanakkale projesi farklı vardiyalı (V1 00-08 → bitis 08:00) → tetik 08:30
  // olmalı, eski schedule bunu kaçırıyordu. Mig 094 (proje seviye vardiya)
  // sonrası N projeli senaryolarda her projenin vardiya bitişlerini bağımsız
  // takip etmek için cron sıklığı artırıldı, dispatch endpoint'e taşındı.
  cron.schedule('*/15 * * * *', async () => {
    try {
