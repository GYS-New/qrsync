...
function initArsivCron() {
  ...
  // Her 5 dakikada: mesai cikis hatirlatma (PT-aktif firmalar icin)
  // Vardiya bitis + 15 dk gectiyse personele "cikis QR unutulmu?" push,
  // + 30 dk gectiyse otomatik kapatir (cikis_tipi = OTOMATIK_ZAMAN_ASIMI).
  cron.schedule('*/5 * * * *', async () => {
    try {
      const response = await fetch(`${getBaseUrl()}/api/cron/mesai-cikis-hatirlatma`, {
        method: 'POST',
        headers: cronHeaders,
      })
      const result = await response.json()
      if ((result.push_gonderilen ?? 0) > 0 || (result.otomatik_kapatilan ?? 0) > 0) {
        console.log('[CRON-MESAI-CIKIS]', new Date().toISOString(), result)
        await logCron('mesai_cikis_hatirlatma', result)
      }
    } catch (error) {
      console.error('[CRON-MESAI-CIKIS] Hata:', error)
    }
  })
  ...
  // NOT: personel_destek cron'u pg_cron'a taşındı (proje-bazlı).
  //   qrsync-personel-destek-renault:   0 21,5,13 * * *  → 00:00, 08:00, 16:00 TRT (V bitis +30dk)
  //   qrsync-personel-destek-canakkale: 30 21,5,13 * * * → 00:30, 08:30, 16:30 TRT (V bitis +30dk)
  // Endpoint ?proje_id=<uuid> parametresi ile spesifik proje icin calisir.
  // Node-cron'da tutmak Railway restart'a duyarli — pg_cron her durumda calisir.
  ...
  // Her saat başı: PDKS kullanilan token temizligi (10 dk+ eski kayitlar)
  cron.schedule('0 * * * *', async () => {
    try {
      const response = await fetch(`${getBaseUrl()}/api/cron/pdks-token-temizle`, {
        method: 'POST',
        headers: cronHeaders,
      })
      const result = await response.json()
      if ((result.silinen ?? 0) > 0) {
        console.log('[CRON-PDKS-TEMIZLE]', new Date().toISOString(), `${result.silinen} eski token silindi`)
      }
    } catch (error) {
      console.error('[CRON-PDKS-TEMIZLE] Hata:', error)
    }
  })

  console.log('[CRON] Arşiv cron job başlatıldı (her 6 saat)')
  console.log('[CRON] PDKS token temizligi başlatıldı (her saat başı)')
  ...
  console.log('[CRON] Personel görev desteği cron başlatıldı (her 15 dk, proje vardiya bitişi penceresi)')
